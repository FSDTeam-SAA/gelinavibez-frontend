...
import { Building2, Settings, LogOut, X, CreditCard, MessageSquare } from "lucide-react";
...
const navigation = [
  { name: "My Apartments", href: "/landlord/my-apartment", icon: Building2 },
  { name: "Tenant Application", href: "/landlord/tenant-application", icon: CreditCard },
  { name: "Message", href: "/landlord/message", icon: MessageSquare },
  { name: "All Admins", href: "/landlord/admin", icon: MessageSquare },
  { name: "Settings", href: "/landlord/landlord-profile", icon: Settings },
];
...
export function Sidebar({ isMobileMenuOpen = false, onClose }: SidebarProps) {
  ...
  const handelGO = async () => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch Stripe dashboard link");
  ...
  const NavLinks = () => (
    <>
      ...
      {getUser.data?.data?.stripeAccountId && (
        <button
          onClick={handelGO}
          className="flex items-center gap-3 px-4 py-3 text-[18px] font-normal text-[#0F3D61] rounded-[4px] hover:bg-[#0F3D6133]/20 transition-colors w-full"
        >

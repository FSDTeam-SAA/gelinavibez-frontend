...
async function fetchLocations(): Promise<string[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/apartment/locations`)
...
export function ApartmentSearch() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.length > 0) setLocation("Location") // reset location if typing
  }
  ...
  const handleLocationSelect = (city: string) => {
    setLocation(city)
    setSearchQuery("") // clear input when selecting location
    setShowLocationDropdown(false)
  }
  ...
  const handleSearch = () => {
    ...
    if (location !== "Location") params.append("q", location)
  ...
  const handleReset = () => {
    ...
    router.push("/apartments") // remove query params
  }
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          ...
          <div className="relative w-full sm:w-48">
            ...
            {showLocationDropdown && (
              <div
                className="absolute top-full left-0 mt-2 w-full min-w-[150px] max-w-[200px] sm:w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-[100]"
                role="listbox"
              >
                {locationsLoading && (
                ...
                {locationsError && (
                  <div className="px-4 py-2 text-sm text-red-600">Failed to load</div>
                )}
            ...
            {showSuggestions && filteredSuggestions.length > 0 && (
              <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-lg shadow-lg border border-gray-200 py-2 max-h-64 overflow-y-auto z-[200]">
                {filteredSuggestions.map((apartment) => (
                  ...
                  >
                    {apartment.title} — {apartment.address.city}, {apartment.address.state}
                  </button>
                ))}
              </div>
            )}

...
interface FormData {
  ...
  clientNumber: string;
  clientEmail: string;
  serviceCategories: string; // stores _id
  serviceAreas: string;
  scopeOfWork: string;
  workHours: string;
  superContact: string;
...
export default function ContractorForm() {
  ...
  // Watch current service selection
  const selectedServiceId = watch("serviceCategories");
  ...
  // Reset service selection if services fail or empty
  useEffect(() => {
    if (servicesError || serviceOptions.length === 0) {
      setValue("serviceCategories", "");
    }
  }, [servicesError, serviceOptions.length, setValue]);
  ...
  const mutation = useMutation({
    mutationFn: async (formData: FormData & { image?: File }) => {
      ...
      payload.append("number", formData.clientNumber);
      payload.append("email", formData.clientEmail);
      payload.append("service", formData.serviceCategories); // This is the _id
      payload.append("serviceAreas", formData.serviceAreas);
      payload.append("scopeWork", formData.scopeOfWork);
      payload.append("worlHour", formData.workHours);
      payload.append("superContact", formData.superContact);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <Select
                  ...
                  disabled={servicesLoading || servicesError || serviceOptions.length === 0}
                  onValueChange={(value) => setValue("serviceCategories", value)}
                >
                  <SelectTrigger
                    id="service-categories"

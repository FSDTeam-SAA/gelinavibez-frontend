...
type UserRef = string | { _id: string; firstName: string; lastName: string; profileImage?: string };
...
export default function ChatWindow({ conversationId, receiverId, onBack }: Props) {
  ...
  const { sendMessage } = useChatSocket({
    userId: currentUserId!,
    conversationId,
    onNewMessage: (msg: Message) => setMessages(prev => [...prev, msg]),
    onMessageSent: (msg: Message) => setMessages(prev => prev.map(m => (m._id === 'temp' ? msg : m))),
  });
  ...
  const isMessageFromMe = (msg: Message) => (typeof msg.senderId === 'string' ? msg.senderId === currentUserId : msg.senderId._id === currentUserId);

  const receiverMsg = messages.find((m) => (typeof m.senderId === 'string' ? m.senderId !== currentUserId : m.senderId._id !== currentUserId));

  const receiverName = receiverMsg && typeof receiverMsg.senderId !== 'string'
          ...
          {messages.map((msg) => {
            ...
            return (
              <div key={msg._id} className={`flex items-end gap-2 max-w-xs lg:max-w-md ${isMe ? 'ml-auto flex-row-reverse' : ''}`}>
                <div className={`rounded-2xl px-4 py-3 ${isMe ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white border rounded-bl-none'}`}>
                  <p>{msg.message}</p>
                  <div className="text-xs mt-1">{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
              </div>
            );
          })}

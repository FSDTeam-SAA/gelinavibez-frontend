...
//     ApiResponse<StripeDashboardResponse>
//   > => {
//     const res = await fetch(
...
//   const { mutate: openStripeDashboard, isPending: dashboardLoading } =
//     useMutation({
...
//     { href: "/services", label: "Contractor " },
...
export function Navbar() {
  ...
  /* --------------------- REQUEST ADMIN ROLE --------------------- */
  const requestAdmin = async (): Promise<ApiResponse<unknown>> => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/request-admin`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.message ?? "Failed to request admin role");
    }
    return res.json();
  };

  const {
    mutate: applyForAdmin,
    isPending: adminRequestLoading,
  } = useMutation({
                ...
                >
                  ...
                  {role === "user" && (
                    <>
                      ...
                      {/* APPLY AS ADMIN - Now with API call */}
                      <DropdownMenuItem

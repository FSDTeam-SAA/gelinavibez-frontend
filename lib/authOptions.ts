...
//           console.log("Login response:", res?.status, res.statusText, res.ok, res);
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          // If login fails, throw the API message so it comes to the frontend
          ...
          // Throw the error to be caught by signIn
